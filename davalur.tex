...
\end{equation}
where a given node point $V_X$ is adjacent to neighbors north, south, east, and west on a regular grid of uniform spacing $h$. We iteratively calculated $V$ by setting each interior point equal to the average of its four surrounding points. The process was repeated until the potential value stopped changing and was stabilized according to some small tolerance $\delta < \num{1e-6}$.
...
This code is available at \url{https://github.com/devangel77b/426davalur-lab10}. Relaxation is implemented at approximately line 48. Line 46 implements pinned nodes for a centerline defect. 
